...
--				30/07/2018 AND s.name NOT IN (''db_denydatawriter'',''db_owner'',''db_accessadmin'',''db_securityadmin'',''db_ddladmin'',''db_backupoperator'',''db_datareader'',''db_datawriter'',''db_denydatareader'')
--				26/07/2021 @LoginName sysname = NULL,@RunningID INT = NULL. Remove Temp tables
-- Description:	Finding a schema as user.
-- =============================================
CREATE PROCEDURE [dbo].[usp_Schema_SchemaAsUser]
...
BEGIN
	...
	SELECT	DISTINCT @RunningID,
			...
			''Schema'' Type,
			NULL ColumnName,
			''DROP SCHEMA '' + QUOTENAME(s.name) + '';'' ConstraintName,
			@Message Message,
			@URL_Reference URL,
			@SeverityName Severity,
			@CheckID
	FROM	' + @DBName + N'sys.sysusers u
			INNER JOIN ' + @DBName + N'sys.schemas s ON s.name = u.name
	WHERE	u.uid > 4
			AND s.name NOT IN (''db_denydatawriter'',''db_owner'',''db_accessadmin'',''db_securityadmin'',''db_ddladmin'',''db_backupoperator'',''db_datareader'',''db_datawriter'',''db_denydatareader'')
	OPTION(RECOMPILE);';

...
-- Description:	Missing GDPR Suport Encryption Original Data Type extended properties
-- =============================================
CREATE PROCEDURE [dbo].[usp_DesignStandard_MissingGDPRSuportEncryptionOriginalDataType]
...
BEGIN
	...
	FROM	' + @DBName + N'sys.tables t
			...
			INNER JOIN ' + @DBName + N'sys.types ty ON ty.system_type_id = c.system_type_id
			LEFT JOIN ' + @DBName + N'sys.extended_properties ep ON ep.major_id = t.object_id
													AND c.column_id = ep.minor_id
													AND ep.name = ''OriginalDataType''
	WHERE   ty.name = ''varbinary''
			AND c.name LIKE ''%Encrypted%''

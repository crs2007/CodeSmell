...
-- =============================================
CREATE PROCEDURE [dbo].[usp_Index_FindNunWithManyColumns]
...
BEGIN
	...
	FROM    ' + @DBName + N'sys.tables t
			...
			CROSS APPLY (SELECT	TOP 1  COUNT_BIG(1) NumberOfColumns
	...
	WHERE   i.type > 1
			AND i.is_unique = 0
			AND S.name != ''sys''
			AND LS.NumberOfColumns /CONVERT(FLOAT,c.NumberOfColumns) > 0.5;';
